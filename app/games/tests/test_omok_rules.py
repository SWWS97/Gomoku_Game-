...
class OmokRuleTests(unittest.TestCase):
    ...
    def test_black_exact_five_true_overline_false(self):
        ...
        self.assertTrue(is_overline([row[:] for row in bd3], 0, 5, BLACK, simulate=True))
    ...
    def test_out_of_bounds_and_occupied_behaviour(self):
        ...
        self.assertFalse(is_forbidden_double_three([row[:] for row in bd], -1, 0, BLACK))
        self.assertFalse(is_forbidden_double_four([row[:] for row in bd], 10, 10, BLACK))
